...
This will search for your project root and save separate tag files for PHP and JavaScript files. You need to create a file named ".vim-tags.proj" and place it in your project root directory. For now, this just tells the plugin which directory is your project root. Creates tag files automatically when you save code files. If you save a .php file, it only creates tags for other .php files in the project, and saves them in their own phptags file. If you save a .js file, it generates tags in a separate jstags file. When you are editing a file, and try to do a code lookup using tags, it will only search one tags file matching the language of the file you are editing, based on file extension. The point of all this is to avoid false positives and keep the tag matches to a minimum.
...
Install it the normal way you install Vim plugins. This also depends on the [vim-elhiv](https://github.com/still-dreaming-1/vim-elhiv/tree/master) plugin, so you will have to install that as well. You need to create a file named ".vim-tags.proj" and place it in your project root directory.
